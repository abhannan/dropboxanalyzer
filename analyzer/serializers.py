...
class UserUsageInfoSerializer(serializers.ModelSerializer):
    ...
    def get_files_categories(self, obj):
        ...
        img_extns = ['ai', 'bmp', 'gif', 'ico', 'jpeg', 'jpg', 'png', 'psd', 'svg', 'tif', 'tiff']
        ...
        categoriest = {'Images': 0, 'Videos': 0, 'Documents': 0, 'Others': 0, 'Audios': 0}

        for key, value in file_categories.items():
            if key in img_extns:
                categoriest['Images'] += value
            elif key in audio_extns:
                categoriest['Audios'] += value
            elif key in video_extns:
                categoriest['Videos'] += value
            elif key in docs_extns:
                categoriest['Documents'] += value
            else:
                categoriest['Others'] += value
        return categoriest
